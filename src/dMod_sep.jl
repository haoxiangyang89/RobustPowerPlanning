function dual_master_sep(fData,uData,hData,T,groupDict,Γ,expansion_factor,vmaxT,vminT,θDmaxT,θDminT,xhat,yhat,zhat,sphat,sqhat)
    ...
    dp_m = direct_model(optimizer_with_attributes(() -> Gurobi.Optimizer(GUROBI_ENV), "NumericFocus" => 3, "BarConvTol" => 1e-6, "MIPGap" => 1e-6, "BarQCPConvTol" => 1e-6,
...
function dual_sub_sep(fData,uData,hData,T,groupDict,Γ,expansion_factor,vmaxT,vminT,θDmaxT,θDminT,
    ...
    "OptimalityTol" => 1e-6, "QCPDual" => 1, "OutputFlag" => 1, "Threads" => 1));
    ...
    @variable(sprob, 0 <= u_dp_cb[m in groupList, t in 2:T] <= 1);
    @variable(sprob, 0 <= u_dm_cb[m in groupList, t in 2:T] <= 1);
    ...
    @variable(sprob, 0 <= u_hp_cb[i in hData.hList, t in 2:T] <= 1);
    @variable(sprob, 0 <= u_hm_cb[i in hData.hList, t in 2:T] <= 1);
    ...
    @constraint(sprob, rdppCons1[i in fData.IDList, t in 2:T], rdpp[i,t] <= fData.cz * u_dp_cb[group_rev[i],t]);
    @constraint(sprob, rdppCons2[i in fData.IDList, t in 2:T], rdpp[i,t] >= -fData.cz * u_dp_cb[group_rev[i],t]);
    @constraint(sprob, rdppCons3[i in fData.IDList, t in 2:T], rdpp[i,t] <= λdp[i,t] + 5 * (1 - u_dp_cb[group_rev[i],t]) * fData.cz);
    @constraint(sprob, rdppCons4[i in fData.IDList, t in 2:T], rdpp[i,t] >= λdp[i,t] - 5 * (1 - u_dp_cb[group_rev[i],t]) * fData.cz);

    @constraint(sprob, rdpmCons1[i in fData.IDList, t in 2:T], rdpm[i,t] <= fData.cz * u_dm_cb[group_rev[i],t]);
    @constraint(sprob, rdpmCons2[i in fData.IDList, t in 2:T], rdpm[i,t] >= -fData.cz * u_dm_cb[group_rev[i],t]);
    @constraint(sprob, rdpmCons3[i in fData.IDList, t in 2:T], rdpm[i,t] <= λdp[i,t] + 5 * (1 - u_dm_cb[group_rev[i],t]) * fData.cz);
    @constraint(sprob, rdpmCons4[i in fData.IDList, t in 2:T], rdpm[i,t] >= λdp[i,t] - 5 * (1 - u_dm_cb[group_rev[i],t]) * fData.cz);

    @constraint(sprob, rdqpCons1[i in fData.IDList, t in 2:T], rdqp[i,t] <= fData.cz * u_dp_cb[group_rev[i],t]);
    @constraint(sprob, rdqpCons2[i in fData.IDList, t in 2:T], rdqp[i,t] >= -fData.cz * u_dp_cb[group_rev[i],t]);
    @constraint(sprob, rdqpCons3[i in fData.IDList, t in 2:T], rdqp[i,t] <= λdq[i,t] + 5 * (1 - u_dp_cb[group_rev[i],t]) * fData.cz);
    @constraint(sprob, rdqpCons4[i in fData.IDList, t in 2:T], rdqp[i,t] >= λdq[i,t] - 5 * (1 - u_dp_cb[group_rev[i],t]) * fData.cz);

    @constraint(sprob, rdqmCons1[i in fData.IDList, t in 2:T], rdqm[i,t] <= fData.cz * u_dm_cb[group_rev[i],t]);
    @constraint(sprob, rdqmCons2[i in fData.IDList, t in 2:T], rdqm[i,t] >= -fData.cz * u_dm_cb[group_rev[i],t]);
    @constraint(sprob, rdqmCons3[i in fData.IDList, t in 2:T], rdqm[i,t] <= λdq[i,t] + 5 * (1 - u_dm_cb[group_rev[i],t]) * fData.cz);
    @constraint(sprob, rdqmCons4[i in fData.IDList, t in 2:T], rdqm[i,t] >= λdq[i,t] - 5 * (1 - u_dm_cb[group_rev[i],t]) * fData.cz);

    @constraint(sprob, rhppCons1[i in hData.hList, t in 2:T], rhpp[i,t] <= fData.cz * u_hp_cb[i,t]);
    @constraint(sprob, rhppCons2[i in hData.hList, t in 2:T], rhpp[i,t] >= -fData.cz * u_hp_cb[i,t]);
    @constraint(sprob, rhppCons3[i in hData.hList, t in 2:T], rhpp[i,t] <= λhp[i,t] + 5 * (1 - u_hp_cb[i,t]) * fData.cz);
    @constraint(sprob, rhppCons4[i in hData.hList, t in 2:T], rhpp[i,t] >= λhp[i,t] - 5 * (1 - u_hp_cb[i,t]) * fData.cz);

    @constraint(sprob, rhpmCons1[i in hData.hList, t in 2:T], rhpm[i,t] <= fData.cz * u_hm_cb[i,t]);
    @constraint(sprob, rhpmCons2[i in hData.hList, t in 2:T], rhpm[i,t] >= -fData.cz * u_hm_cb[i,t]);
    @constraint(sprob, rhpmCons3[i in hData.hList, t in 2:T], rhpm[i,t] <= λhp[i,t] + 5 * (1 - u_hm_cb[i,t]) * fData.cz);
    @constraint(sprob, rhpmCons4[i in hData.hList, t in 2:T], rhpm[i,t] >= λhp[i,t] - 5 * (1 - u_hm_cb[i,t]) * fData.cz);

    @constraint(sprob, rhppzCons11[i in hData.hList, t in 2:T], rhppz1[i,t] <= fData.cz * u_hp_cb[i,t]);
    @constraint(sprob, rhppzCons12[i in hData.hList, t in 2:T], rhppz1[i,t] >= -fData.cz * u_hp_cb[i,t]);
    @constraint(sprob, rhppzCons13[i in hData.hList, t in 2:T], rhppz1[i,t] <= λhzp1[i,t] + 5 * (1 - u_hp_cb[i,t]) * fData.cz);
    @constraint(sprob, rhppzCons14[i in hData.hList, t in 2:T], rhppz1[i,t] >= λhzp1[i,t] - 5 * (1 - u_hp_cb[i,t]) * fData.cz);

    @constraint(sprob, rhpmzCons11[i in hData.hList, t in 2:T], rhpmz1[i,t] <= fData.cz * u_hm_cb[i,t]);
    @constraint(sprob, rhpmzCons12[i in hData.hList, t in 2:T], rhpmz1[i,t] >= -fData.cz * u_hm_cb[i,t]);
    @constraint(sprob, rhpmzCons13[i in hData.hList, t in 2:T], rhpmz1[i,t] <= λhzm1[i,t] + 5 * (1 - u_hm_cb[i,t]) * fData.cz);
    @constraint(sprob, rhpmzCons14[i in hData.hList, t in 2:T], rhpmz1[i,t] >= λhzm1[i,t] - 5 * (1 - u_hm_cb[i,t]) * fData.cz);

    @constraint(sprob, rhppzCons21[i in hData.hList, t in 2:T], rhppz2[i,t] <= fData.cz * u_hp_cb[i,t]);
    @constraint(sprob, rhppzCons22[i in hData.hList, t in 2:T], rhppz2[i,t] >= -fData.cz * u_hp_cb[i,t]);
    @constraint(sprob, rhppzCons23[i in hData.hList, t in 2:T], rhppz2[i,t] <= λhzp3[i,t] + 5 * (1 - u_hp_cb[i,t]) * fData.cz);
    @constraint(sprob, rhppzCons24[i in hData.hList, t in 2:T], rhppz2[i,t] >= λhzp3[i,t] - 5 * (1 - u_hp_cb[i,t]) * fData.cz);

    @constraint(sprob, rhpmzCons21[i in hData.hList, t in 2:T], rhpmz2[i,t] <= fData.cz * u_hm_cb[i,t]);
    @constraint(sprob, rhpmzCons22[i in hData.hList, t in 2:T], rhpmz2[i,t] >= -fData.cz * u_hm_cb[i,t]);
    @constraint(sprob, rhpmzCons23[i in hData.hList, t in 2:T], rhpmz2[i,t] <= λhzm3[i,t] + 5 * (1 - u_hm_cb[i,t]) * fData.cz);
    @constraint(sprob, rhpmzCons24[i in hData.hList, t in 2:T], rhpmz2[i,t] >= λhzm3[i,t] - 5 * (1 - u_hm_cb[i,t]) * fData.cz);
    ...
    @constraint(sprob, udp_constr[m in groupList, t in 2:T], u_dp_cb[m,t] == u_dp_val[m,t]);
    @constraint(sprob, udm_constr[m in groupList, t in 2:T], u_dm_cb[m,t] == u_dm_val[m,t]);
    @constraint(sprob, uhp_constr[i in hData.hList, t in 2:T], u_hp_cb[i,t] == u_hp_val[i,t]);
    @constraint(sprob, uhm_constr[i in hData.hList, t in 2:T], u_hm_cb[i,t] == u_hm_val[i,t]);
...
function dual_sub_sep_p(fData,uData,hData,T,groupDict,Γ,expansion_factor,vmaxT,vminT,θDmaxT,θDminT,
    xhat,yhat,zhat,sphat,sqhat,u_dp_val,u_dm_val,u_hp_val,u_hm_val)
    # obtain the master problem for the dual
    θu = Dict();
    for t in 1:T
    ...
    @variable(subd, λeqn_u_dp[i in groupList, t in 2:T]);
    @variable(subd, λeqn_u_dm[i in groupList, t in 2:T]);
    @variable(subd, λeqn_u_hp[i in hData.hList, t in 2:T]);
    @variable(subd, λeqn_u_hm[i in hData.hList, t in 2:T]);
    @variable(subd, λu_dp_u[i in groupList, t in 2:T] <= 0);
    @variable(subd, λu_dm_u[i in groupList, t in 2:T] <= 0);
    @variable(subd, λu_hp_u[i in hData.hList, t in 2:T] <= 0);
    @variable(subd, λu_hm_u[i in hData.hList, t in 2:T] <= 0);
    ...
    @constraint(subd, dp_cal[i in fData.IDList, t in 2:T], dp_var[i,t] - pi_rdpp[i,t,3] + pi_rdpp[i,t,4] - pi_rdpm[i,t,3] + pi_rdpm[i,t,4] == uData[i].DP0[t]);
    @constraint(subd, dq_cal[i in fData.IDList, t in 2:T], dq_var[i,t] - pi_rdqp[i,t,3] + pi_rdqp[i,t,4] - pi_rdqm[i,t,3] + pi_rdqm[i,t,4] == uData[i].DQ0[t]);
    ...
    @constraint(subd, hp_cal1[i in fData.IDList, t in 2:T; i in hData.hList], h_var[i,t] - uData[i].RESP0[t]*(1+expansion_factor * z[i]) - 
                uData[i].RESP0[t]*uData[i].RESPmax*expansion_factor*uz_hp[i,t] + uData[i].RESP0[t]*uData[i].RESPmin*expansion_factor*uz_hm[i,t] -
                pi_rhpp[i,t,3] + pi_rhpp[i,t,4] - pi_rhpm[i,t,3] + pi_rhpm[i,t,4] <= 0);
    @constraint(subd, hp_cal2[i in fData.IDList, t in 2:T; !(i in hData.hList)], h_var[i,t] == 0.0);

    # uzhp/uzhm constraints
    @constraint(subd, uzp_linear1[i in hData.hList, t in 2:T], uz_hp[i,t] <= pi_rhppz1[i,t,3] - pi_rhppz1[i,t,4]);
    @constraint(subd, uzp_linear2[i in hData.hList, t in 2:T], uz_hp[i,t] <= z[i]);
    @constraint(subd, uzp_linear3[i in hData.hList, t in 2:T], uz_hp[i,t] >= pi_rhppz2[i,t,3] - pi_rhppz2[i,t,4] + z[i] - 1);
    @constraint(subd, uzm_linear1[i in hData.hList, t in 2:T], uz_hm[i,t] <= pi_rhpmz1[i,t,3] - pi_rhpmz1[i,t,4]);
    @constraint(subd, uzm_linear2[i in hData.hList, t in 2:T], uz_hm[i,t] <= z[i]);
    @constraint(subd, uzm_linear3[i in hData.hList, t in 2:T], uz_hm[i,t] >= pi_rhpmz2[i,t,3] - pi_rhpmz2[i,t,4] + z[i] - 1);

    Mdual = 5*fData.cz;
    # auxiliary r constraints
    @constraint(subd, rdpp_cons[i in fData.IDList, t in 2:T], pi_rdpp[i,t,1] - pi_rdpp[i,t,2] + pi_rdpp[i,t,3] - pi_rdpp[i,t,4] - (uData[i].DPmax[t] - uData[i].DP0[t]) == 0);
    @constraint(subd, rdpm_cons[i in fData.IDList, t in 2:T], pi_rdpm[i,t,1] - pi_rdpm[i,t,2] + pi_rdpm[i,t,3] - pi_rdpm[i,t,4] - (uData[i].DPmin[t] - uData[i].DP0[t]) == 0);
    @constraint(subd, rdqp_cons[i in fData.IDList, t in 2:T], pi_rdqp[i,t,1] - pi_rdqp[i,t,2] + pi_rdqp[i,t,3] - pi_rdqp[i,t,4] - (uData[i].DQmax[t] - uData[i].DQ0[t]) == 0);
    @constraint(subd, rdqm_cons[i in fData.IDList, t in 2:T], pi_rdqm[i,t,1] - pi_rdqm[i,t,2] + pi_rdqm[i,t,3] - pi_rdqm[i,t,4] - (uData[i].DQmin[t] - uData[i].DQ0[t]) == 0);
    @constraint(subd, rhpp_cons[i in hData.hList, t in 2:T], pi_rhpp[i,t,1] - pi_rhpp[i,t,2] + pi_rhpp[i,t,3] - pi_rhpp[i,t,4] - uData[i].RESP0[t]*uData[i].RESPmax == 0);
    @constraint(subd, rhpm_cons[i in hData.hList, t in 2:T], pi_rhpm[i,t,1] - pi_rhpm[i,t,2] + pi_rhpm[i,t,3] - pi_rhpm[i,t,4] + uData[i].RESP0[t]*uData[i].RESPmin == 0);
    @constraint(subd, rhppz1_cons[i in hData.hList, t in 2:T], pi_rhppz1[i,t,1] - pi_rhppz1[i,t,2] + pi_rhppz1[i,t,3] - pi_rhppz1[i,t,4] == 1);
    @constraint(subd, rhppz2_cons[i in hData.hList, t in 2:T], pi_rhppz2[i,t,1] - pi_rhppz2[i,t,2] + pi_rhppz2[i,t,3] - pi_rhppz2[i,t,4] == 1);
    @constraint(subd, rhpmz1_cons[i in hData.hList, t in 2:T], pi_rhpmz1[i,t,1] - pi_rhpmz1[i,t,2] + pi_rhpmz1[i,t,3] - pi_rhpmz1[i,t,4] == 1);
    @constraint(subd, rhpmz2_cons[i in hData.hList, t in 2:T], pi_rhpmz2[i,t,1] - pi_rhpmz2[i,t,2] + pi_rhpmz2[i,t,3] - pi_rhpmz2[i,t,4] == 1);

    # u constraints
    @constraint(subd, udp_cons[m in groupList, t in 2:T], sum(-fData.cz*(pi_rdpp[i,t,1] + pi_rdpp[i,t,1] + pi_rdqp[i,t,1] + pi_rdqp[i,t,2]) + 
        Mdual*(pi_rdpp[i,t,3] + pi_rdpp[i,t,4] + pi_rdqp[i,t,3] + pi_rdqp[i,t,4]) for i in groupDict[2][m]) + λeqn_u_dp[m,t] + λu_dp_u[m,t] <= 0);
    @constraint(subd, udm_cons[m in groupList, t in 2:T],sum(-fData.cz*(pi_rdpm[i,t,1] + pi_rdpm[i,t,1] + pi_rdqm[i,t,1] + pi_rdqm[i,t,2]) + 
        Mdual*(pi_rdpm[i,t,3] + pi_rdpm[i,t,4] + pi_rdqm[i,t,3] + pi_rdqm[i,t,4]) for i in groupDict[2][m]) + λeqn_u_dm[m,t] + λu_dm_u[m,t] <= 0);
    @constraint(subd, uhp_cons[i in hData.hList, t in 2:T], -fData.cz*(pi_rhpp[i,t,1] + pi_rhpp[i,t,1] + pi_rhppz1[i,t,1] + pi_rhppz1[i,t,2] + pi_rhppz2[i,t,1] + pi_rhppz2[i,t,2]) + 
        Mdual*(pi_rhpp[i,t,3] + pi_rhpp[i,t,4] + pi_rhppz1[i,t,3] + pi_rhppz1[i,t,4] + pi_rhppz2[i,t,3] + pi_rhppz2[i,t,4]) + λeqn_u_hp[i,t] + λu_hp_u[i,t] <= 0);
    @constraint(subd, uhm_cons[i in hData.hList, t in 2:T], -fData.cz*(pi_rhpm[i,t,1] + pi_rhpm[i,t,1] + pi_rhpmz1[i,t,1] + pi_rhpmz1[i,t,2] + pi_rhpmz2[i,t,1] + pi_rhpmz2[i,t,2]) + 
        Mdual*(pi_rhpm[i,t,3] + pi_rhpm[i,t,4] + pi_rhpmz1[i,t,3] + pi_rhpmz1[i,t,4] + pi_rhpmz2[i,t,3] + pi_rhpmz2[i,t,4]) + λeqn_u_hm[i,t] + λu_hm_u[i,t] <= 0);
    ...
    @objective(subd, Min, fData.cz*sum(lpplus[i,t] + lpminus[i,t] + lqplus[i,t] + lqminus[i,t] for i in fData.IDList for t in 2:T) + 
        ...
        Mdual*(sum(pi_rdpp[i,t,3] + pi_rdpp[i,t,4] + pi_rdpm[i,t,3] + pi_rdpm[i,t,4] + 
                   pi_rdqp[i,t,3] + pi_rdqp[i,t,4] + pi_rdqm[i,t,3] + pi_rdqm[i,t,4] for i in fData.IDList for t in 2:T) + 
               sum(pi_rhpp[i,t,3] + pi_rhpp[i,t,4] + pi_rhpm[i,t,3] + pi_rhpm[i,t,4] + 
                   pi_rhppz1[i,t,3] + pi_rhppz1[i,t,4] + pi_rhpmz1[i,t,3] + pi_rhpmz1[i,t,4] + 
                   pi_rhppz2[i,t,3] + pi_rhppz2[i,t,4] + pi_rhpmz2[i,t,3] + pi_rhpmz2[i,t,4] for i in hData.hList for t in 2:T)) - 
        sum(λeqn_u_dp[i,t]*u_dp_val[i,t] + λeqn_u_dm[i,t]*u_dm_val[i,t] + λu_dp_u[i,t] + λu_dm_u[i,t] for i in groupList for t in 2:T) -
        sum(λeqn_u_hp[i,t]*u_hp_val[i,t] + λeqn_u_hm[i,t]*u_hm_val[i,t] + λu_hp_u[i,t] + λu_hm_u[i,t] for i in hData.hList for t in 2:T));
