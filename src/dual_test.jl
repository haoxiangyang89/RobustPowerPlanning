for t in 2:T
    for i in fData.IDList
        set_lower_bound(subp[:λyIni][i,t],-9999999);
        set_upper_bound(subp[:λyIni][i,t],9999999);
    end
end

for t in 2:T
    for k in fData.brList
        set_lower_bound(subp[:λxIni][k,t],-9999999);
        set_upper_bound(subp[:λxIni][k,t],9999999);
    end
end

for t in 2:T
    for i in fData.genIDList
        set_lower_bound(subp[:λrampu][i,t],-9999999);
        set_upper_bound(subp[:λrampu][i,t],9999999);
    end
end

for i in hData.hList
    set_lower_bound(subp[:λzIni][i],-9999999);
    set_upper_bound(subp[:λzIni][i],9999999);
end
...
function createSecond_dual(fData,uData,hData,T,groupDict,Γ,expansion_factor,vmaxT,vminT,θDmaxT,θDminT,xhat,yhat,zhat,sphat,sqhat)
    ...
    @variable(sprob,0 <= λv[i in fData.IDList, t in 2:T] <= 9999999);
    @variable(sprob,0 <= λvu[i in fData.IDList, t in 2:T] <= 9999999);
    @variable(sprob,-9999999 <= λvl[i in fData.IDList, t in 2:T] <= 0);

    # dual variables for θ difference bounds
    @variable(sprob,0 <= λθ1[k in fData.brList, t in 2:T] <= 9999999);
    @variable(sprob,-9999999 <= λθ2[k in fData.brList, t in 2:T] <= 0);
    @variable(sprob,-9999999 <= λθRef[t in 2:T] <= 9999999);

    # dual variables for generations
    @variable(sprob,0 <= λspu[i in fData.genIDList, t in 1:T] <= 9999999);
    @variable(sprob,-9999999 <= λspl[i in fData.genIDList, t in 1:T] <= 0);
    @variable(sprob,0 <= λsqu[i in fData.genIDList, t in 1:T] <= 9999999);
    @variable(sprob,-9999999 <= λsql[i in fData.genIDList, t in 1:T] <= 0);
    @variable(sprob,-9999999 <= λspIni[i in fData.genIDList] <= 9999999);
    @variable(sprob,-9999999 <= λsqIni[i in fData.genIDList] <= 9999999);
    @variable(sprob,0 <= λrampu[i in fData.genIDList, t in 2:T] <= 9999999);
    @variable(sprob,-9999999 <= λrampl[i in fData.genIDList, t in 2:T] <= 0);

    # dual variables for cosine relaxation
    @variable(sprob,0 <= λcs1[k in fData.brList, t in 2:T] <= 9999999);
    @variable(sprob,-9999999 <= λcs2[k in fData.brList, t in 2:T] <= 0);
    @variable(sprob,-9999999 <= λcs3[k in fData.brList, t in 2:T] <= 0);
    # dual variables for sine relaxation
    @variable(sprob,0 <= λss1[k in fData.brList, t in 2:T] <= 9999999);
    @variable(sprob,-9999999 <= λss2[k in fData.brList, t in 2:T] <= 0);
    @variable(sprob,0 <= λss4[k in fData.brList, t in 2:T] <= 9999999);
    @variable(sprob,-9999999 <= λss5[k in fData.brList, t in 2:T] <= 0);
    # dual variables for wc/ws/cs/ss/vv/l equalities of reverse flows
    @variable(sprob,-9999999 <= λwce[k in fData.brList, t in 2:T] <= 9999999);
    @variable(sprob,-9999999 <= λwse[k in fData.brList, t in 2:T] <= 9999999);
    @variable(sprob,-9999999 <= λcse[k in fData.brList, t in 2:T] <= 9999999);
    @variable(sprob,-9999999 <= λsse[k in fData.brList, t in 2:T] <= 9999999);
    @variable(sprob,-9999999 <= λvve[k in fData.brList, t in 2:T] <= 9999999);

    # dual variables for power flow equations
    @variable(sprob,0 <= λptrans1[fData.brList, t in 2:T] <= 9999999);
    @variable(sprob,-9999999 <= λptrans2[fData.brList, t in 2:T] <= 0);
    @variable(sprob,0 <= λqtrans1[fData.brList, t in 2:T] <= 9999999);
    @variable(sprob,-9999999 <= λqtrans2[fData.brList, t in 2:T] <= 0);

    # dual variables for the SOC inequalities
    @variable(sprob,-9999999 <= μ1[k in fData.brList,j in 1:2, t in 2:T; !(fData.rateA[k]==Inf)] <= 9999999);
    @variable(sprob,-9999999 <= μ2[k in fData.brList,j in 1:3, t in 2:T] <= 9999999);
    @variable(sprob,-9999999 <= μ3[k in fData.brList,j in 1:2, t in 2:T] <= 9999999);
    @variable(sprob,-9999999 <= μ4[i in fData.IDList,j in 1:2, t in 2:T] <= 9999999);
    @variable(sprob,-9999999 <= μ5[k in fData.brList,j in 1:4, t in 2:T] <= 9999999);
    @variable(sprob,0 <= ν1[k in fData.brList, t in 2:T; !(fData.rateA[k]==Inf)] <= 9999999);
    @variable(sprob,0 <= ν2[k in fData.brList, t in 2:T] <= 9999999);
    @variable(sprob,0 <= ν3[k in fData.brList, t in 2:T] <= 9999999);
    @variable(sprob,0 <= ν4[i in fData.IDList, t in 2:T] <= 9999999);
    @variable(sprob,0 <= ν5[k in fData.brList, t in 2:T] <= 9999999);

    # dual variables for vv McCormick
    @variable(sprob,-9999999 <= λvv1[k in fData.brList, t in 2:T] <= 0);
    @variable(sprob,-9999999 <= λvv2[k in fData.brList, t in 2:T] <= 0);
    @variable(sprob,0 <= λvv3[k in fData.brList, t in 2:T] <= 9999999);
    @variable(sprob,0 <= λvv4[k in fData.brList, t in 2:T] <= 9999999);
    # dual variables for wc McCormick
    @variable(sprob,-9999999 <= λwc1[k in fData.brList, t in 2:T] <= 0);
    @variable(sprob,-9999999 <= λwc2[k in fData.brList, t in 2:T] <= 0);
    @variable(sprob,0 <= λwc3[k in fData.brList, t in 2:T] <= 9999999);
    @variable(sprob,0 <= λwc4[k in fData.brList, t in 2:T] <= 9999999);
    # dual variables for ws McCormick
    @variable(sprob,-9999999 <= λws1[k in fData.brList, t in 2:T] <= 0);
    @variable(sprob,-9999999 <= λws2[k in fData.brList, t in 2:T] <= 0);
    @variable(sprob,0 <= λws3[k in fData.brList, t in 2:T] <= 9999999);
    @variable(sprob,0 <= λws4[k in fData.brList, t in 2:T] <= 9999999);

    # dual variables for tangent constraints
    @variable(sprob,0 <= λtangent1[k in fData.brList, t in 2:T] <= 9999999);
    @variable(sprob,-9999999 <= λtangent2[k in fData.brList, t in 2:T] <= 0);
    # dual variables for LNC constraints
    @variable(sprob,-9999999 <= λlnc1[k in fData.brList, t in 2:T] <= 0);
    @variable(sprob,-9999999 <= λlnc2[k in fData.brList, t in 2:T] <= 0);

    # dual variables for battery dynamics
    @variable(sprob,0 <= λbat_lim[i in fData.IDList, t in 1:T] <= 9999999);
    @variable(sprob,-9999999 <= λbat_trans[i in fData.IDList, t in 2:T] <= 9999999);
    @variable(sprob,-9999999 <= μbat_out[i in fData.IDList, j in 1:2, t in 2:T] <= 9999999);
    @variable(sprob,0 <= νbat_out[i in fData.IDList, t in 2:T] <= 9999999);
    @variable(sprob,0 <= λbat_eff[i in fData.IDList, l in 1:length(bData.ηα[i]), t in 2:T] <= 9999999);
    ...
    @variable(sprob, -9999999 <= λdp[i in fData.IDList, t in 2:T] <= 9999999);
    @variable(sprob, -9999999 <= λdq[i in fData.IDList, t in 2:T] <= 9999999);
    @variable(sprob, u_dp[m in groupList, t in 2:T], Bin);
    @variable(sprob, u_dm[m in groupList, t in 2:T], Bin);
    @variable(sprob, -9999999 <= rdpp[i in fData.IDList, t in 2:T] <= 9999999);
    @variable(sprob, -9999999 <= rdpm[i in fData.IDList, t in 2:T] <= 9999999);
    @variable(sprob, -9999999 <= rdqp[i in fData.IDList, t in 2:T] <= 9999999);
    @variable(sprob, -9999999 <= rdqm[i in fData.IDList, t in 2:T] <= 9999999);

    # dual variables for renewable generation variables
    @variable(sprob, 0 <= λhp[i in fData.IDList, t in 2:T] <= 9999999);
    @variable(sprob, 0 <= λuzpu[i in hData.hList, t in 2:T] <= 9999999);
    @variable(sprob, 0 <= λuzmu[i in hData.hList, t in 2:T] <= 9999999);
    @variable(sprob, 0 <= λhzp1[i in fData.IDList, t in 2:T] <= 9999999);
    @variable(sprob, 0 <= λhzp2[i in fData.IDList, t in 2:T] <= 9999999);
    @variable(sprob, -9999999 <= λhzp3[i in fData.IDList, t in 2:T] <= 0);
    @variable(sprob, 0 <= λhzm1[i in fData.IDList, t in 2:T] <= 9999999);
    @variable(sprob, 0 <= λhzm2[i in fData.IDList, t in 2:T] <= 9999999);
    @variable(sprob, -9999999 <= λhzm3[i in fData.IDList, t in 2:T] <= 0);
    @variable(sprob, u_hp[i in hData.hList, t in 2:T], Bin);
    @variable(sprob, u_hm[i in hData.hList, t in 2:T], Bin);
    @variable(sprob, -9999999 <= rhpp[i in hData.hList, t in 2:T] <= 9999999);
    @variable(sprob, -9999999 <= rhpm[i in hData.hList, t in 2:T] <= 9999999);
    @variable(sprob, 0 <= rhppz1[i in hData.hList, t in 2:T] <= 9999999);
    @variable(sprob, -9999999 <= rhppz2[i in hData.hList, t in 2:T] <= 0);
    @variable(sprob, 0 <= rhpmz1[i in hData.hList, t in 2:T] <= 9999999);
    @variable(sprob, -9999999 <= rhpmz2[i in hData.hList, t in 2:T] <= 0);

    # dual variables for non-anticipitavity constraints
    @variable(sprob, -9999999 <= λxIni[k in fData.brList, t in 2:T] <= 9999999);
    @variable(sprob, -9999999 <= λyIni[i in fData.IDList] <= 9999999);
    @variable(sprob, -9999999 <= λzIni[i in hData.hList] <= 9999999);
